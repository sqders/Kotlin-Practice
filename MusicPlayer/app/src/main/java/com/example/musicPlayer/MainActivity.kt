        ...
        if(cursor != null)
            if(cursor.moveToFirst())
                do{
                    ...
                    val album_idC = cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.Media.ALBUM_ID))
                    val artistC = cursor.getString(cursor.getColumnIndex(MediaStore.Audio.Media.ARTIST))
                    val pathC = cursor.getString(cursor.getColumnIndex(MediaStore.Audio.Media.DATA))
                    val durationC = cursor.getLong(cursor.getColumnIndex(MediaStore.Audio.Media.DURATION))
                    val uri = Uri.parse("content://media/external/audio/albumart")
                    val artUriC = Uri.withAppendedPath(uri, album_idC.toString()).toString()
                    val music = Music(
                        id =idC,
                        title =titleC,
                        artist =artistC,
                        path = pathC, duration = durationC, album = albumC, artUri =artUriC )
